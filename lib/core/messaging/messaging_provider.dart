...
class MessagingNotifier extends Notifier<MessagingState> {
  ...
  MessagingState build() {
    ...
    ref.onDispose(() {
      ...
      for (final t in _lockReleaseTimers.values) {
        t.cancel();
      }
  ...
  Future<void> _processDecryptedPlaintext(
    ...
    if (plaintext is DomainTextMessage) {
      ...
      _completeSecureHandshake(endpointId, peerId, 'capabilities_exchange_success');
  ...
  Future<void> _handleSecureIdentityReceived(SessionSecureIdentity identity) async {
    ...
    try {
      ...
      final currentPeerSession = state.sessions[identity.peerId];
      if (currentPeerSession != null) {
        state = state.copyWith(
          sessions: {
            ...state.sessions,
            identity.peerId: currentPeerSession.copyWith(
              status: SessionStatus.disconnected,
              isSecure: false,
            ),
          },
    ...
    // Immediately verify that the map contains identity.peerId
    if (_securitySessions.containsKey(identity.peerId)) {
      print('[SECURITY] SECURITY_SESSION_REGISTERED\npeerId=${identity.peerId}\nendpointId=${identity.endpointId}');
      print('[FORENSIC][SESSION_REGISTERED] peerId=${identity.peerId} endpointId=${identity.endpointId} isDeviceA=${secSession.isDeviceA} sessionId=${secSession.sessionId}');
      
      // Cancel and remove _handshakeTimers[identity.endpointId]
      final timer = _handshakeTimers.remove(identity.endpointId);
      if (timer != null) {
        timer.cancel();
        print('[HANDSHAKE][WATCHDOG_CANCEL]\nreason=SECURE_SESSION_REGISTERED\nendpointId=${identity.endpointId}');
        print('[FORENSIC][WATCHDOG_CANCELLED] endpointId=${identity.endpointId} reason=SECURE_SESSION_REGISTERED');
      } else {
        print('[FORENSIC][WATCHDOG_ALREADY_GONE] endpointId=${identity.endpointId} reason=no_timer_found_to_cancel');
      }
    } else {
      print('[SECURITY] ERROR: Security session registration verification failed for peerId=${identity.peerId}');
      print('[FORENSIC][SESSION_REGISTER_FAILED] peerId=${identity.peerId} endpointId=${identity.endpointId}');
    }
    ...
    if (!isV1) {
      if (derivedKeys.isDeviceA) {
        ...
        print('[FORENSIC][CAPS_EXCHANGE_START] isDeviceA=false peerId=${identity.peerId} endpointId=${identity.endpointId} role=RESPONDER_WAITING');
      }
    } else {
    ...
    for (final peer in discoveredList) {
      ...
      repo.getPeer(peerId).then((dbPeer) {
        if (dbPeer != null && dbPeer.trustState == PeerTrustState.trusted) {
          ...
          print('[VANTRA][CONNECTION][ATTEMPT_START]\npeerId=$peerId\nendpointId=${peer.endpointId}');
          print('[VANTRA][CONNECTION][DEBUG] CONNECT_REQUEST_START endpointId=${peer.endpointId} peerId=$peerId localRole=INITIATOR discovered=${_lastDiscoveredList.any((p) => p.endpointId == peer.endpointId)} connecting=${peer.isConnecting} connected=${peer.isConnected}');
          ...
          final currentAttemptToken = (_connectAttemptTokens[peerId] ?? 0) + 1;
          _connectAttemptTokens[peerId] = currentAttemptToken;

          ref.read(peerDiscoveryServiceProvider).connect(
            peer.endpointId,
            localName: '${localIdentity.displayName}:${localIdentity.peerId}',
          ).then((_) {
            print('[VANTRA][CONNECTION][WAITING_NATIVE]\npeerId=$peerId\nendpointId=${peer.endpointId}');
            print('[VANTRA][CONNECTION][DEBUG] CONNECT_REQUEST_SUCCESS endpointId=${peer.endpointId} peerId=$peerId');
            print('[VANTRA][CONNECTION] REQUEST_SUCCESS: endpointId=${peer.endpointId}');
            
            _lockReleaseTimers[peerId]?.cancel();
            _lockReleaseTimers[peerId] = Timer(const Duration(seconds: 30), () {
              if (_connectAttemptTokens[peerId] != currentAttemptToken) {
                print('[VANTRA][CONNECTION][DEBUG] Stale connection attempt timeout ignored for peerId=$peerId');
                return;
              }
              print('[VANTRA][CONNECTION][TIMEOUT]\npeerId=$peerId\nendpointId=${peer.endpointId}');
              final currentSession = state.sessions[peerId];
              final isNowConnected = currentSession != null &&
                  (currentSession.status == SessionStatus.connected ||
                   currentSession.status == SessionStatus.handshaking) &&
                  currentSession.endpointId == peer.endpointId;
              if (!isNowConnected) {
                _releaseConnectLock(peerId, 'timeout');
              }
            });
          }).catchError((e) {
            _releaseConnectLock(peerId, 'connect_error');
            print('[VANTRA][CONNECTION][DEBUG] CONNECT_REQUEST_EXCEPTION endpointId=${peer.endpointId} peerId=$peerId error=$e');
  ...
  void _handleConnectionUpdate(ConnectionUpdate update) {
    ...
    final resolvedEndpointName = update.endpointName.contains(':')
        ? update.endpointName
        : (_endpointNames[update.endpointId] ?? update.endpointName);
    final index = resolvedEndpointName.lastIndexOf(':');
    final candidateName = index != -1 ? resolvedEndpointName.substring(0, index) : resolvedEndpointName;
    final candidatePeerId = index != -1 ? resolvedEndpointName.substring(index + 1) : null;
    ...
    if (update.status == ConnectionStatus.connected) {
      ...
      if (pId != null) {
        _releaseConnectLock(pId, 'result_connected');
      }
      ...
      if (!isAlreadyConnected) {
        ...
        _handshakeTimers[update.endpointId] = Timer(handshakeWatchdogDuration, () async {
          ...
          if (pId != null) {
            ...
            if (currentSecSession != null) {
              print('[HANDSHAKE][WATCHDOG_SKIP_SECURE]\npeerId=$pId\nendpointId=${update.endpointId}');
              return;
            }
      ...
      print('[VANTRA][CONNECTION][INITIATED]\nendpointId=${update.endpointId}');
      ...
      if (candidatePeerId != null && candidatePeerId.isNotEmpty) {
        ...
        print('[VANTRA][CONNECTION] ROLE_DECISION localPeerId=$localPeerId remotePeerId=$remotePeerId role=$role');
        _releaseConnectLock(candidatePeerId, 'initiated');
      }
      ...
      if (peerId != null) {
        ...
        if (isStale) {
          ...
          _releaseConnectLock(peerId, 'update_${update.status.name}');
  ...
  void _releaseConnectLock(String peerId, String reason) {
    final wasRemoved = _activeConnectLocks.remove(peerId);
    if (wasRemoved) {
      print('[VANTRA][CONNECTION][LOCK_RELEASE]\npeerId=$peerId\nreason=$reason');
    }
    _lockReleaseTimers[peerId]?.cancel();
    _lockReleaseTimers.remove(peerId);
    _connectAttemptTokens.remove(peerId);
  }

  void _completeSecureHandshake(String endpointId, String peerId, String source) {
    print('[VANTRA][HANDSHAKE][COMPLETE] endpoint=$endpointId peerId=$peerId source=$source');
    
    if (_handshakeTimers.containsKey(endpointId)) {
    ...
    print('[VANTRA][HANDSHAKE][STATE_SYNC] sessionsCount=${state.sessions.length} endpointMappingsCount=${state.endpointToPeerId.length}');
  }
